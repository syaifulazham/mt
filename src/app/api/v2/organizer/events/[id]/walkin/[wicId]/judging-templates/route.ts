...
// GET /api/v2/organizer/events/[id]/walkin/[wicId]/judging-templates
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string; wicId: string }> }) {
  ...
  const rows = await db.eventWalkInCompetitionJudgingTemplate.findMany({
...
// POST /api/v2/organizer/events/[id]/walkin/[wicId]/judging-templates
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; wicId: string }> }) {
  ...
  try {
    await db.eventWalkInCompetitionJudgingTemplate.create({
      data: { walkInCompetitionId: wicId, judgingTemplateId },
    });
  } catch {
