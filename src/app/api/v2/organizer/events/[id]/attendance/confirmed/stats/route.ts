  ...
  try {
    const [summaryRows, gradeRows, stateRows] = await Promise.all([
      db.$queryRaw<SummaryRow[]>`
        SELECT
          ...
          COUNT(DISTINCT t.id) FILTER (WHERE te."attendedAt" IS NOT NULL)                                              AS teams_attended
        ${fromJoins}
        WHERE 1=1
      `,
    ...
    return NextResponse.json({
      summary: {
        ...
        teamsAttended:     Number(s?.teams_attended     ?? 0),
      },
      ...
      byState: stateRows.map((r) => ({
        ...
        teamsAttended:     Number(r.teams_attended),
      })),
    });
  } catch (e) {
    console.error("[attendance/confirmed/stats]", e);
