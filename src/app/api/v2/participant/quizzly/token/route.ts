...
export async function POST(req: NextRequest) {
  ...
  const participant = await db.participant.findUnique({
    ...
    select: {
      id: true, eduLevel: true, classGrade: true, ppki: true, age: true,
      quizzlyAccess: true,
    },
  });
  ...
  const ec = await db.eventCompetition.findUnique({
    ...
    select: {
      id: true, quizzlySessionId: true, quizzlyAssignBy: true, quizzlyQuizMap: true,
  ...
  try {
    ...
    // One row per (participant, event-competition): tokens are single-use, so a
    // re-issue supersedes whatever was there.
    const row = await db.participantQuizzlyToken.upsert({
      where:  { participantId_eventCompetitionId: { participantId: participant.id, eventCompetitionId: ec.id } },
      create: {
        participantId: participant.id, eventCompetitionId: ec.id,
