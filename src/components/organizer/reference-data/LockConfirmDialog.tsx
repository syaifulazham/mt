...
export function LockConfirmDialog({ open, itemName, isLocked, onConfirm, onClose }: Props) {
  ...
  useEffect(() => {
    if (open) { setCode(genCode()); setInput(""); }
  }, [open]);
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-4 py-1">
          <p className="text-sm text-zinc-600">
            {isLocked
              ? "Buka kunci akan membenarkan pengeditan semula pada rekod ini."
              : "Mengunci rekod ini akan menghalang sebarang pengeditan atau pemadaman."}
          </p>
